...
const CartPage = () => {
  ...
  const getToken = async () => {
    try {
      const { data } = await axios.get(" https://ecommerce-website-beta-inky.vercel.app/api/v1/product/braintree/token");
  ...
  const handlePayment = async () => {
    try {
      ...
      const { data } = await axios.post(" https://ecommerce-website-beta-inky.vercel.app/api/v1/product/braintree/payment", {
        nonce,
        cart,
      });
      setLoading(false);
